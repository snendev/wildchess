...
use bevy::prelude::{resource_added, App, Commands, IntoSystemConfigs, Plugin, Res, Update};

use crate::{
    components::{PieceBundle, Team},
    GamePieces, Movement, PieceEvent, Promotion, RequestPromotion,
};

mod board;
pub use board::WildBoardPlugin;

mod capture;
mod movement;
mod targets;

fn initialize_board(mut commands: Commands, pieces: Res<GamePieces>) {
    for (config, start_positions) in pieces.0.clone().into_iter() {
        ...
        let pieces = start_positions
            ...
            .map(move |(position, team)| PieceBundle::from_configuration(&config, position, team));
...
impl Plugin for GameplayPlugin {
    fn build(&self, app: &mut App) {
        app.add_event::<PieceEvent<Movement>>()
            ...
            .add_systems(
                Update,
                initialize_board.run_if(resource_added::<GamePieces>()),
            );
    }
}
