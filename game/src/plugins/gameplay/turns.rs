...
use crate::{
    components::{Behavior, Player, Position, Promotable, Team, Turn},
    events::{IssueMoveEvent, RequestPromotionEvent},
    IssuePromotionEvent, TurnEvent,
};

pub fn detect_turn(
    piece_query: Query<(&Team, Option<&Promotable>)>,
    ...
    for IssueMoveEvent(movement) in move_reader.iter() {
        if let Ok((team, promotable)) = piece_query.get(movement.entity) {
