// A quick fork of bevy_replicon_renet that relies on renet2.
...
use bevy_ecs::prelude::{
    resource_added, resource_exists, resource_removed, EventReader, EventWriter, IntoSystemConfigs,
    IntoSystemSetConfigs, Res, ResMut,
};
...
impl RepliconRenetServerPlugin {
    ...
    fn forward_server_events(
        mut renet_server_events: EventReader<renet2::ServerEvent>,
        mut server_events: EventWriter<ServerEvent>,
    ) {
        for event in renet_server_events.read() {
            let replicon_event = match event {
                renet2::ServerEvent::ClientConnected { client_id } => {
                    ServerEvent::ClientConnected {
                        client_id: ClientId::new(client_id.raw()),
                    }
                }
                renet2::ServerEvent::ClientDisconnected { client_id, reason } => {
    ...
    fn receive_packets(
        ...
        for client_id in connected_clients.iter_client_ids() {
            let renet_client_id = renet2::ClientId::from_raw(client_id.get());
    ...
    fn send_packets(
        ...
        for (client_id, channel_id, message) in replicon_server.drain_sent() {
            let client_id = renet2::ClientId::from_raw(client_id.get());
